...
function Login({navigation}: any) {
  ...
  const handleLogin = async (data: any) => {
    setLoading(true);
    const err = await relativeLogin(String(data.email), String(data.password));
    setError(err);
    setLoading(false);
  };
  ...
  return (
    <>
      ...
      <TopBigIconLayout
        ...
        <SugradoDividerText text="veya" style={styles.divider_text} />
        <Button
          mode="contained-tonal"
          onPress={() => {
            navigation.navigate(PAGE_NAMES.RELATIVE.AUTH.REGISTER);
          }}
          theme={{dark: false}}
          icon="account-plus">
          Kayıt Ol
